...
// Iden为函数名称，Fuction1为函数头定义　Function2为函数体（包括定义变量）
bool Function()
{
	if(judge("function"))
	{
...
bool Statement()
{
	...
	{
		if(strlen(outlabel)!=0)
		{
			output("Jump",outlabel,"-","-");
		}
		else
		{
			cout <<"break place is wrong ! no loop!"<<endl;
			return false;
		}
	}
	else if(IO())//IO语句
...
bool IBT()
{
	if(judge("If")||judge("if"))
	{
		if(BoolExpr())
		{
			if(!(judge("Then")||judge("then")))
			{
...
bool WBD()
{
	...
	if(judge("While")||judge("while"))
	{
		...
		copystr(temp3,outlabel);
		copystr(outlabel,temp2);
		if(BoolExpr())
		{
			output("if",temp2,"-","-");
			if(judge("Do")||judge("do"))
			{
				if(!Statement())
				{
					copystr(outlabel,temp3);
					return false;
				}
				output("Jump",temp1,"-","-");
				output("label",temp2,"-","-");
